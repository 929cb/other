...
unsigned short lcd_read_panelid(void)
{
	...
	idio_config.direction = GPIO_DigitalOutput;
	...
	idio_config.direction = GPIO_DigitalInput;
	gpio_init(GPIO3, 12, idio_config);
	gpio_init(GPIO3, 20, idio_config);
	gpio_init(GPIO3, 28, idio_config);

	idx = (unsigned char)gpio_readPin(GPIO3, 28); 	/* 读取M0 */
	idx |= (unsigned char)gpio_readPin(GPIO3, 20) << 1;	/* 读取M1 */
	idx |= (unsigned char)gpio_readPin(GPIO3, 12) << 2;	/* 读取M2 */
...
void lcdgpio_init(void)
{
	...
	/* GPIO初始化 */
	gpio_config.direction = GPIO_DigitalOutput;			/* 输出 			*/
	gpio_config.outputLogic = 1; 							/* 默认关闭背光 */
	gpio_init(GPIO1, 8, gpio_config);						/* 背光默认打开 */
	gpio_writePin(GPIO1, 8, 1);								/* 打开背光     */
}
